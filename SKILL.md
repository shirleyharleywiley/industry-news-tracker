...
> **来源**：这些铁律来自 2026.7.3 蓝牙耳机报告事故（用户两次质疑）。**任何 agent 违反任一条，Agent 8 边界守卫直接判定 ✗ fail，必须重做。**
...
2. ❌ 严禁用纯文字描述代替链接
3. ❌ 严禁 Agent 返回"二手事件摘要"时偷放进报告
4. ✅ 正确处理：补搜 → 找不到则**删除该条目** + 在文末"已删除事件清单"明示
5. 任何形式的"占位符"都是不可接受的。

### 铁律 3：流程执行
1. **Agent 7 输出后必须立刻跑 Agent 8**——Agent 8 检查全部 7 项（日期/覆盖度/源质量/5要素/合并/链接/链接完整性），任一 fail 不可输出。
2. **Agent 8 pass 后必须跑 Agent 9**——3 个终止条件全部满足才可输出最终报告。
3. **主 agent 不可跳过 Agent 8/9**——这是硬性流程，2026.7.3 事故就是因为跳过了。
...
- **检查 7：链接完整性 🔴**——每条必须有可点击 URL，模糊描述直接 ⛔ fail（覆盖铁律 2）
